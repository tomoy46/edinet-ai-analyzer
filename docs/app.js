...
const state = { disclosures: [], status: {}, sample: false, saved: loadFavorites() };
...
function render() {
  ...
  }).sort((a, b) => sortOrder === "importance"
    ? b.importance - a.importance || b.published_at.localeCompare(a.published_at)
  ...
  $("#disclosureList").innerHTML = filtered.map((item) => {
    ...
    return `<article class="disclosure${saved ? " saved" : ""}">
      <div class="meta"><span>発表日時</span><br><time>${formatDate(item.published_at)}</time><div class="company">${escapeHtml(item.security_code)} · ${escapeHtml(item.company_name)}</div></div>
      ...
      <div class="actions"><a href="${encodeURI(item.pdf_url)}" target="_blank" rel="noopener noreferrer">TDnet PDF ↗</a><button class="star-button${saved ? " active" : ""}" data-code="${escapeHtml(item.security_code)}" aria-label="お気に入りを切替">${saved ? "★" : "☆"}</button></div>
    </article>`;
  }).join("");
...
["#searchInput", "#categoryFilter", "#importanceFilter", "#sortOrder", "#savedOnly"].forEach((selector) => $(selector).addEventListener("input", render));
